 ...
 * Description:
 * -----------
 * This script records API calls that match a certain KFL statement. Recorded API calls are stored in 
 * JSON arrays, in files, in AWS S3. 
 * Files are uploaded to AWS S3 once every hour.
 * 
 * The script expects AWS S3 specific credentials.
 ...
 *
 * See an example for a configuration clause in this folder
 *
 * How to use:
 * -----------
...
function onItemCaptured(data) {
    ...
    if ( kfl.match(env.APICALLS_KFL, data) ){
        ...
        // console.log("Found event match!", JSON.stringify(data);
    }
}
...
function apicallsJob(){
    ...
    if (apicallsArr.length > 0){
        try{
            ...
            vendor.s3.put(
                env.APICALLS_S3_BUCKET, apicallsFile, env.AWS_REGION, 
